...
## Abstract
LeGO-LOAM, a lightweight and ground-optimized lidar odometry and mapping method, is able to do six degree-of-freedom pose estimation for real time with ground vehicles. This project we evaluate the LeGO-LOAM which reduced computational expense while keeping similar accuracy compared to LOAM method using codes provided by [RobustFieldAutonomyLab/LeGO-LOAM](https://github.com/RobustFieldAutonomyLab/LeGO-LOAM) and [laboshinl's version of LOAM](https://github.com/laboshinl/loam_velodyne). The data used for evaluation is raw data (with Velodyne LiDAR data and IMU) from [KITTI Odometry Benchmark Dataset](http://www.cvlibs.net/datasets/kitti/), [UTBM Dataset](https://epan-utbm.github.io/utbm_robocar_dataset/) and [KAIST Urban Dataset](http://irap.kaist.ac.kr/dataset/). We compared the mapping and odometry results of these data, and analysis the relative motion and mapping error in this report. 
